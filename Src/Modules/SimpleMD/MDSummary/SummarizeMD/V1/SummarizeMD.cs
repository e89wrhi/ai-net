    ...
    public IEndpointRouteBuilder MapEndpoint(IEndpointRouteBuilder builder)
    {
        builder.MapPost($"{EndpointConfig.BaseApiPath}/md/summarize",
                async (SummarizeMDRequestDto request, IMediator mediator, IHttpContextAccessor httpContextAccessor, CancellationToken cancellationToken) =>
                {
                    ...
                    var command = new SummarizeMDCommand(userId, request.Instruction, request.ModelId);
                    var result = await mediator.Send(command, cancellationToken);
                    return Results.Ok(new SummarizeMDResponseDto(result.Response, result.ModelId, result.ProviderName));
                })
            ...
            .WithDescription("Uses AI to summarize a markdown context file. Pass an instruction such as 'bullet points' or 'one paragraph' to control the output format.")
            .WithOpenApi()
            .HasApiVersion(1.0);
