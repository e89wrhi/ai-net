...
public class ImageGrpcService : ImageCaption.ImageGrpcService.ImageGrpcServiceBase
{
    private readonly IMediator _mediator;
    private readonly IServiceProvider _serviceProvider;

    public ImageGrpcService(IMediator mediator, IServiceProvider serviceProvider)
    {
        _mediator = mediator;
    ...
    public override async Task<GetImageCaptionsResponse> GetImageCaptions(GetImageCaptionsRequest request, ServerCallContext context)
    {
        ...
        try
        {
            var readDb = _serviceProvider.GetService<ImageCaption.Data.ImageReadDbContext>();
            if (readDb != null)
            {
