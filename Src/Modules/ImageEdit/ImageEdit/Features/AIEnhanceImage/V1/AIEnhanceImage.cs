...
public record AIEnhanceImageCommand(string ImageUrlOrBase64, string Prompt) : ICommand<AIEnhanceImageResult>;

public record AIEnhanceImageResult(Guid SessionId, Guid ResultId, string ResultImageUrl);
...
internal class AIEnhanceImageHandler : ICommandHandler<AIEnhanceImageCommand, AIEnhanceImageResult>
{
    ...
    public async Task<AIEnhanceImageResult> Handle(AIEnhanceImageCommand request, CancellationToken cancellationToken)
    {
        ...
        return new AIEnhanceImageResult(sessionId.Value, resultId.Value, resultImageUrl);
    }
}
