...
public record AnalyzeCodeCommand(string Code, ProgrammingLanguage Language) : ICommand<AnalyzeCodeResult>;

public record AnalyzeCodeResult(Guid SessionId, Guid ReportId, string Summary, int IssueCount);
...
internal class AnalyzeCodeHandler : ICommandHandler<AnalyzeCodeCommand, AnalyzeCodeResult>
{
    ...
    public async Task<AnalyzeCodeResult> Handle(AnalyzeCodeCommand request, CancellationToken cancellationToken)
    {
